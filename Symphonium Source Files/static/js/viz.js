...
(function () {
	...
	a.init = function() {
		...
		var s = {
			...
			// example playlist
			playlist: ['Blank Space.mp3', 'wildest dreams.mp3'],
			...
			trigger: 'spin',											// default visualization
	...
	a.loadSound = function() {
		...
		if (navigator.userAgent.search("Safari") >= 0 && navigator.userAgent.search("Chrome") < 0) {
			...
			var path = '/static/mp3/'+State.playlist[0];
	...
	a.loadSoundAJAX = function() {
	    ...
	    request.open("GET", "/static/mp3/"+State.playlist[0], true);
	...
	h.changeSong = function(direction) {
		...
		if (State.soundCloudData) {
			...
			if (audio) {
				audio.src = '/static/mp3/'+State.playlist[Math.abs(State.currentSong)%State.playlist.length];
				h.readID3(audio.src);
			}
		}
